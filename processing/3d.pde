...
float OFFSET_X = 2.5, OFFSET_Y = -2.5; 
// These offsets are chip specific, and vary. 
// Play with them to get the best ones for you
 
//This sets up the Processing 3 Java window dimension and visuals
void setup() {
    size(400, 300, P3D);
    sp = new Serial(this, "/dev/cu.wchusbserial1410", 9600); 
    //You have to rename your port in the "" and change the baud rate.
    buff = new byte[128];
    r = new float[3];
}
float protz, protx;
void draw() {
    ...
    translate(400/2, 300/2, -400); 
    //Sets left/right, up/down, and toward/away translation
    ...
    //It will only read until 10 bytes of data
    String mystr = (new String(buff, 0, bytes)).trim(); 
    //trim removes whitespace at start of string
    if(mystr.split(" ").length != 3) { 
        //uses string to break characters by using a delimiter ""
        println(mystr);
        return;
    }
    setVals(r, mystr);
    ...
    protx = x;
    println(r[0] + ", " + r[1] + ", " + r[2]); 
    //prints x, y, z values in the console
}
